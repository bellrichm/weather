...
namespace BellRichM.Weather.Api.Controllers
{
    ...
    public class ConditionsController : ApiController
    {
        ...
        public async Task<IActionResult> GetMinMaxConditionsByHour([FromQuery] int startDayOfYear, [FromQuery] int endDayOfYear, [FromQuery] int offset, [FromQuery] int limit)
        {
            _logger.LogEvent(EventId.ConditionsController_GetMinMaxConditionsByHour, "{@startDayOfYear} {@endDayOfYear} {@offset} {@limit}", startDayOfYear, endDayOfYear, offset, limit);
            if (!ModelState.IsValid)
            {
                _logger.LogDiagnosticInformation("{@ModelState}", ModelState);
                var errorResponseModel = CreateModel();
            ...
            var minMaxGroupPage = await _conditionService.GetMinMaxConditionsByHour(startDayOfYear, endDayOfYear, offset, limit).ConfigureAwait(true);
